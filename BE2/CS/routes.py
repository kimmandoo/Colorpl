...
def update_member_status(member_id: int, member_update: schemas.MemberUpdateStatus, db: Session = Depends(get_db), current_administrator: models.Administrator = Depends(super_admin_only)):
...
def get_reviews(skip: int = 0, limit: int = 100, content: str = Query(None), db: Session = Depends(get_db), current_administrator: models.Administrator = Depends(admin_only)):
    if content:
        reviews = crud.get_reviews_by_content(db, content=content, skip=skip, limit=limit)
    else:
        reviews, total_count = crud.get_reviews(db, skip=skip, limit=limit)
    return reviews
...
def spoiler_review(review_id: int, is_spoiler: bool = Query(...), management_reason: str = Query(...), db: Session = Depends(get_db), current_administrator: Administrator = Depends(chief_or_super_admin)):
    db_review = crud.update_review(db, review_id, is_spoiler, management_by=current_administrator.email, management_reason=management_reason)
    if db_review is None:
        raise HTTPException(status_code=404, detail="Review not found")
    return db_review
